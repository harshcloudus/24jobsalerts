...
/**
 * AdGateModal — fullscreen interstitial overlay.
 * Used as the final fallback when the Google Ad Placement API has no fill.
 * Mount once in the root layout. Triggered via the "adgate:show-interstitial"
 * custom event dispatched by adManager._showCustomModal().
 */
...
export default function AdGateModal() {
  ...
  // Countdown timer — enables close button after COUNTDOWN_S seconds
  useEffect(() => {
    if (!open) return;
  ...
  // Push AdSense display ad once per open
  useEffect(() => {
    ...
    try {
      ...
      // adsbygoogle not ready yet — slot stays blank, UX still works
    }
  }, [open]);

  // Escape key to close once countdown finishes
  useEffect(() => {
  ...
  return (
    <>
      {/* Backdrop — clickable only after countdown */}
      <div
        ...
        <div className="adgate-container">
          ...
          <div className="adgate-header">
            <span className="adgate-label">
              <span className="material-symbols-rounded adgate-ad-icon" aria-hidden="true">
                campaign
              </span>
              Advertisement
            </span>
            ...
            >
              {canClose
                ...
                : <span className="adgate-countdown" aria-live="polite">{seconds}</span>
              }
            </button>
          </div>
          ...
          <p className="adgate-note">
            This ad supports free job listings on 24jobsalerts.
          </p>
        </div>
      </div>
    </>
